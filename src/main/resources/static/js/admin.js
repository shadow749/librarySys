    ...
    $('#table').bootstrapTable({
        ...
        columns: [ {
            ...
            formatter: function (data, row, index) {
                var temp = ""
                temp = "<a href='javascript:void(0)' onclick=edit('" + data + "','" + row.name + "','" + row.password + "','" + row.card +"')>编辑</a>  &nbsp;&nbsp;";
...
function edit(id, name, pass,card) {
    $("#userName").val(name);
...
function dele(id) {
    Ewin.confirm({message: "确认要删除该账号吗？"}).on(function (e) {
        if (!e) {
            return;
        }
        $.ajax({
